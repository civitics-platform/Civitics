 ...
 * this output is FIX-375 (queued follow-up).
 *
 * Usage:
...
async function main(): Promise<void> {
  ...
  md += `DB host: \`${host}\`. Feeds the FIX-375 merge spec; ingest-side\n`;
  ...
  md += `(FIX-375) must therefore pick clusters whose internal rows share at\n`;
  ...
  md += `out-of-scope for FIX-375 — those collide via the FIX-248 promotion\n`;
