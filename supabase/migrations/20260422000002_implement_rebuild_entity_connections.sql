-- =============================================================================
-- FIX-100 — implement public.rebuild_entity_connections() body
--
-- Stage 1 · 06 left this as a stub returning an empty result set so references
-- compiled during dual-write. Post-cutover the table is empty (0 rows) and the
-- graph is dead until this function actually derives edges.
--
-- Derivation rules (per L5 — entity_connections is derivation-only in Phase 1):
--
--   donation         ← financial_relationships (relationship_type='donation')
--   vote_yes/no/abstain ← votes (latest disposition per official×proposal)
--   co_sponsorship   ← proposal_cosponsors (active — date_withdrawn IS NULL)
--   appointment      ← career_history (is_government=true, has governing_body_id)
--   oversight        ← agencies × governing_bodies static lookup (agencies.governing_body_id)
--   holds_position   ← financial_relationships (owns_stock|owns_bond|property, ended_at IS NULL)
--   gift_received    ← financial_relationships (gift|honorarium)
--   contract_award   ← financial_relationships (contract|grant)
--   lobbying         ← financial_relationships (lobbying_spend)
--
-- Strategy: full rebuild (DELETE + INSERT). L5 says no manual edges in Phase 1,
-- so wholesale rebuild is the simplest correct semantics; runs inside the
-- function's implicit transaction so readers either see the previous snapshot
-- or the new one, never a partial state.
--
-- Many source tables are still empty (financial_relationships, career_history,
-- proposal_cosponsors) — those rules return 0 until FIX-101 re-runs the
-- deferred pipelines. Votes are the only fully-populated source today
-- (217k rows on Pro 2026-04-22).
-- =============================================================================
...
BEGIN
  ...
  -- One edge per (official, proposal): take the LATEST vote (final disposition
  -- supersedes earlier procedural votes). Skip 'present', 'not_voting',
  -- 'paired_*' — those don't represent a stance for the graph.
  WITH ranked AS (
    SELECT
      v.official_id,
  ...
  -- Multiple roles in the same governing body collapse into one edge.
  WITH agg AS (
    SELECT
      ch.official_id,
...
COMMENT ON FUNCTION public.rebuild_entity_connections() IS
  'FIX-100 — derives entity_connections from source tables (financial_relationships, votes, proposal_cosponsors, career_history, agencies). Full rebuild semantics (DELETE + INSERT) per L5. Returns one row per derivation rule with the edge count it produced. Idempotent. Safe to call repeatedly; nightly cron invokes after source pipelines complete.';

-- DOWN:
--   -- Restore the stub from migration 20260421000005 by CREATE OR REPLACE
--   -- against an empty body. Not auto-reversible because the stub's text is
--   -- not preserved here.
