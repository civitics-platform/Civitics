-- =============================================================================
-- FIX-100 follow-up — finalize rebuild_entity_connections()
--
-- Two changes after live testing on Pro:
--
--   1. Combine the four post-vote-INSERT COUNT queries into one query that
--      uses INSERT ... RETURNING + COUNT FILTER. Saves ~3 extra scans of
--      the freshly-inserted 124k vote edges.
--
--   2. Pin a 60s statement_timeout on the function itself via
--      `ALTER FUNCTION ... SET statement_timeout`. Pro's default for the
--      service_role is 8s through PostgREST; the rebuild legitimately runs
--      ~10-15s with current data and will only grow once FIX-101 lands the
--      deferred pipelines (financial_relationships, career_history,
--      cosponsors). Function-scoped setting overrides the role default for
--      this RPC only — other queries keep the safe default.
-- =============================================================================
...
-- Function-scoped statement_timeout. Service_role default through PostgREST
-- is 8s on Pro; the rebuild legitimately needs ~15s today and more once FIX-101
-- lands. Override is only in effect when this function is called.
